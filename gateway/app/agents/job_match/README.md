...
Facade / Orchestrator 协调 Quick Insight、每轮 Workspace intent routing 与 Specialist Strategy；
...
- `quick_insight()` 返回页面浮层使用的 typed `Insight` 与本地化 Prompt Shortcuts，不创建
  Message 或 Artifact。
- `stream_chat()` 为每条用户消息运行 Intent Router，依次产生进度、可见 reply delta 和一个
  完整终态候选结果。
...
20 条 Message。旧本地 Workspace schema 由 Extension 丢弃，不进入 Router 或 Specialist。

## 结构

```text
job_match/
├── agent.py                 # Facade / Orchestrator；stream orchestration 与结果归一化
├── context.py               # 不可变的请求级 JobChatContext
├── intent_router.py         # IntentRouter；选择 Specialist、输出模式与执行指令
...
## Prompt Shortcut 与 Intent Router 边界

Quick Insight 只声明本地化、可编辑的 composer draft。Shortcut 点击不会调用 Agent，发送
时也不会提交 Shortcut id。每一条最终用户消息都经过同一规划路径：

```text
current message + artifact metadata + recent text history
  -> IntentRouter selects Specialist + output mode + instruction
...
Router 只拥有 Specialist、`reply | artifact` 与简短执行指令，不生成正文。具体 Strategy 只拥有
本场景内容生成权，不决定 HTTP、wire protocol、Message/Attachment identity 或 persistence。
这是 Facade + Strategy 的边界，避免 UI 控件成为后端隐藏路由参数。

`IntentRouter.specialists` 是唯一的 Specialist Map。Router 根据 Map 中每个 Strategy 的
`description` 与 `allowed_modes` 动态生成候选列表，并负责校验和解析；`JobMatchAgent`
不再保存重复 Map。

- Analyze Prompt 要求分析，因此 Router 选择 `JobAnalysisAgent + reply`。
...
Router 只读取当前 Artifact 元数据。用户对已有产物发出“短一点”“更自信”“翻译成
英文”等直接编辑指令时进入 Artifact 更新模式；询问“应该怎么改”时仍返回普通回复。

Specialist 始终同时收到原始用户消息与 Router 的执行指令；原始消息优先。历史只传最近
4 条纯文本，不传历史 Attachment。`ResumeTailoringAgent` 只读取当前 CV draft，
`CoverLetterAgent` 只读取当前 Cover Letter draft。
...
1. 实现 `JobMatchSpecialist` Strategy，声明稳定 id、description 与允许的 `OutputMode`。
2. 把实例加入传给 `IntentRouter` 的 Map；删除 Map 项即可停用。
3. 内置 Specialist 在 `JobMatchAgent._build_specialists()` 完成默认装配。
