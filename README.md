...
Agent Bridge combines a Chrome extension, a gateway, and AI agents. The browser first shows a focused **Quick Insight**; an editable Prompt Shortcut then opens a persistent Side Panel where the user can finish the task without repeating the page context.

```text
LinkedIn / Indeed job page
  ↓ right-click
Quick Insight
  ↓ choose a Prompt Shortcut
...
- Offer four editable job Prompt Shortcuts: **Analyze**, **Tailor Resume**, **Generate Cover Letter**, and **Ask More**. General webpages offer **Ask More**.
- Prefill the Shortcut prompt in the Workspace without sending it automatically, so the user can review or edit every request first.
- Open every Shortcut for the same page in one Side Panel Workspace, with one chronological history and inline CV or Cover Letter Attachments.
- Preserve that Workspace locally for the same signed-in user and normalized webpage resource.
- Plan every Workspace turn from the user's final message, shared history, and current Artifacts. No Shortcut ID is sent as an Agent routing instruction.
...
Workspace history and the latest CV / Cover Letter state are stored in the current Chrome profile, not as server-side Threads or Artifacts. A Workspace permits ten user turns and keeps the corresponding Assistant replies, so a fresh Workspace can contain 20 messages. At the limit, the composer and Shortcuts are disabled, but the user can still read the complete history and copy generated content.
...
4. Read the Quick Insight and choose a Prompt Shortcut. Its localized prompt fills the Side Panel composer but is never sent automatically.
5. Review or edit the draft, then send it. The message, current Artifacts, and shared history determine whether the agent replies or creates an Artifact.
...
- **One resource, one Workspace:** Prompt Shortcuts share one local history for the signed-in user and normalized webpage.
- **Intent before output:** Shortcuts only prepare editable drafts; the user's final message determines whether the result is advice or a formal Artifact.
...
- Shared Side Panel history across Prompt Shortcuts
