...
Quick Insight 与 Workspace 使用两个明确的结构化 `Protocol`，避免继续用同一个
`execute() -> DocumentContent` 表达不同场景：
...
`RegisteredAgent` 是两个 Protocol 的交集，注册表中的对象必须同时满足两个接口。
`JobMatchAgent` 实现两个接口。`SummaryPageAgent` 同样实现 `WorkspaceAgent`，其
`handle_chat()` 只允许 `Ask More` 并只返回 `ReplyResult`；因此普通网页也使用同一个新版
`/tasks/workspace` wire contract，不再从 Workspace 返回 `DocumentContent`。
...
Task Service 编排 Agent 调用，并通过独立 reducer 完成一次完整状态转换：
