...
状态：设计已确认，等待书面 Spec 审核。本设计只描述目标实现，不表示云端网关或 Chrome 扩展已经发布。
...
- 已部署 legacy `POST /tasks` 保持由 `modules/task/legacy/` 隔离服务。
...
新版 `JobMatchAgent.handle_chat() -> ChatResult` 不替代旧 `/tasks -> DocumentContent` 契约：

- `POST /tasks/quick-insight` 调用新版 `JobMatchAgent.quick_insight()`。
- `POST /tasks/workspace` 调用新版 `JobMatchAgent.handle_chat()`。
- 已部署的 `POST /tasks` 继续由 `modules/task/legacy/` 入口和 `LegacyJobMatchAgentAdapter` 生成 `DocumentContent`。
- 现有 `gateway/app/agents/job_match.py` 转为 `gateway/app/agents/job_match/` package 时，旧 `execute()` 行为移动到明确命名的 `legacy.py`，不能混入 Orchestrator 的新接口。
- Legacy 响应可以继续包含旧 `DocumentContent` 字段；Workspace 响应不得因此重新引入 `document/html/sections`。

两条路径可以复用纯 Prompt helper，但不能共享 wire response schema 或依靠运行时类型猜测场景。
...
1. 部署支持新 Workspace contract 的网关。
2. 发布使用新 contract 的 Extension。
3. legacy `/tasks` 按现有兼容边界继续服务旧扩展。

网关和 Extension 不同时发布时，Extension 必须把 schema 不匹配响应当作失败，不能用不完整数据覆盖本地 Workspace。
