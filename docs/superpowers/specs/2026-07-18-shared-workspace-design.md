...
扩展用响应中的 `histories` 和 `document` 整体替换本地 Workspace 状态。后端不建立可恢复的 Workspace Thread，只执行一次确定的状态转换；当前内部用户阶段仍会把本次任务的完整 Prompt 和结果写入任务明细，面向公开用户上线前必须补齐脱敏、访问与保留周期策略。

`ask_more` 只追加共享历史，不产生新文档。请求已带 `currentDocument` 时，后端必须在响应中回传规范化后的现有文档；只有 Workspace 尚无产物时才返回 `document: null`。因此切换到 `Ask More` 不会清空后续简历或 Cover Letter 编辑所需的草稿。
...
- 2xx 响应不是合法 JSON，或缺少完整 `histories` / `document` / Workspace 身份：按失败处理，不覆盖本地状态。
