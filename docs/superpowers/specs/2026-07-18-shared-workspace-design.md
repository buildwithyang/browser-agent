...
`quickInsight` 是只读的首屏上下文，`actions` 保存后端最近一次声明的能力。`messages` 按时间保存用户消息和 Agent 回复。`latestDocument` 保存最新产物，供 Cover Letter、Resume 等连续修改。
...
新接口 `POST /tasks/workspace` 接收完整的 Follow-up 上下文；旧 `POST /tasks/current-task` 在迁移期保留为 deprecated alias：
...
新接口不接受 `agent`：后端根据页面上下文完成 Context Routing，`AgentName` 只作为网关内部枚举。`resourceUrl` 只用于关联客户端 Workspace，不作为授权依据；后端根据 `url` 再次规范化并校验它。Side Panel 每次发送前从当前标签页重新采集 Page Context，不把整页正文长期保存在 Workspace。后端保持 Agent 无状态，每次使用请求携带的页面上下文、共享历史和当前用户的服务端简历完成生成。
...
Quick Insight response with Workspace descriptor
  ...
  → POST current-task with shared histories
  → append assistant reply
  → update latestDocument
...
- Workspace 写入失败：保留当前内存状态并提示本地保存失败。
- 网关请求失败：不写入失败的 assistant 消息，保留用户输入以便重试。
- Action 已被后端移除：刷新 Quick Insight Actions，并回退该页面的默认 Action。
