...
- pure-v4 canonical history 只包含完整、按顺序排列的 User/Assistant pair。请求发送前最多
  有 9 个 pair；第 10 轮成功后最多保存 20 条 history，输入框和 Shortcut 禁用。
...
Extension 本地 Workspace 使用 schema v3，状态保存
`resourceUrl/pageTitle/quickInsight/shortcuts/histories/artifacts/updatedAt`，不包含
`actions/selectedActionId`。v2 从未发布；v1 和 v2 都不做转换：`WORKSPACE_GET` 遇到精确旧
schema record 或指向非 v3 record 的 mapping 时，只删除该 record 与 mapping 并返回未连接状态；
下一次 Quick Insight seed 才为当前 owner/resource 创建新的空 v3 Workspace。禁止扫描其他
owner 或 resource，旧 histories 和 Artifacts 不保留。
...
- 第 10 个用户消息允许成功，第 11 个被前后端拒绝；pending、失败恢复、计数器、禁用状态和
  中英文上限提示正确。pure-v4 状态只接受完整 pair，最多 20 条 history。
- v1/v2 Workspace 不转换；`WORKSPACE_GET` 丢弃精确旧 record 和 mapping 并返回未连接，
  下一次 Quick Insight seed 创建全新的空 v3 Workspace。
