...
`actions/selectedActionId`。v1/v2 从未发布，不做转换：遇到精确旧 schema record 或指向
非 v3 record 的 mapping 时，只删除该 record 与 mapping，再为当前 owner/resource 创建新的
空 v3 Workspace；禁止扫描其他 owner 或 resource，旧 histories 和 Artifacts 不保留。
...
- v1/v2 Workspace 不转换；精确旧 record 和 mapping 被丢弃，并创建全新的空 v3 Workspace。
