...
- 全新 v4 Workspace 第 10 轮成功后最多保存 20 条 history，输入框和 Shortcut 禁用。
- 从 v2 迁移的 Workspace 可能额外保留最多 11 条旧 Action Assistant history，因此兼容
  上限为 31 条；无论历史总数多少，仍只允许最多 10 条 `role=user` 消息。
...
- 第 10 个用户消息允许成功，第 11 个被前后端拒绝；pending、失败恢复、计数器、禁用状态和
  中英文上限提示正确。全新 v4 状态最多 20 条 history，v2 迁移状态兼容最多 31 条且旧历史
  不丢失。
