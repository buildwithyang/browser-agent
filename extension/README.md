...
- **后端上下文路由**:LinkedIn、Indeed 页面都只校验 host,并要求用户选中完整 JD(至少 1000 个字符),才分流到 `job_match`。任一条件不满足时回退到 `summary_page`。路由规则在后端更新,无需重新发布扩展。
...
2. 打开任意网页。如需岗位匹配,请在 LinkedIn 或 Indeed 页面选中完整 JD(至少 1000 个字符)。
