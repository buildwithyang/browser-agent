...
Workspace 历史以及最新 CV / Cover Letter state 只保存在当前 Chrome 配置中，不建立服务端 Thread 或 Artifact。pure protocol v4 只保存完整的 User/Assistant pair，最多支持 10 次用户发送和 20 条消息；打开旧的本地 Workspace 时会丢弃旧状态并断开 Side Panel，下一次 Quick Insight 才创建全新 Workspace。达到上限后输入框、Shortcut 与发送按钮会禁用，但仍可阅读完整历史并复制已生成内容。
