...
- 从 Quick Insight 点击 Analyze、Resume、Cover Letter 会立即执行；Ask More 只打开 Workspace 并等待问题。
- 同一页面的所有 Action 进入一个 Side Panel Workspace，共享一份按时间排列的历史，CV 与 Cover Letter 作为生成它们的 Assistant Message 的 Attachment 展示。
- 按“登录用户 + 规范化网页资源”在当前 Chrome 配置中恢复 Workspace。
- Workspace 中的 Action 是强意图提示，不是强制产物命令。Agent 可以先回答简历问题，只有用户明确要求时才创建或更新 CV。
...
Workspace 历史以及最新 CV / Cover Letter state 只保存在当前 Chrome 配置中，不建立服务端 Thread 或 Artifact。每次请求的“已有历史 + 当前消息”最多 10 条；最后一次合法请求的 Assistant 回复仍会保留，因此最终本地时间线最多可有 11 条。
...
4. 阅读 Quick Insight，并选择下一步 Action。Analyze、Resume、Cover Letter 会立即执行，Ask More 等待用户输入。
5. 在 Side Panel 中继续。切换 Action 只提示下一条消息的方向；Agent 仍结合消息和上下文判断是普通回复还是正式产物。
...
- **意图优先：** Action 提示 Orchestrator 当前方向，用户消息决定最终返回建议还是正式 Artifact。
